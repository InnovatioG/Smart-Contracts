...
testRedeemer  getValidator _ ctx (LedgerApiV2.Spending txOutRef, redeemer) =
    ...
    in case txInInfo of
        Just !info ->
            let
                -- -- !_ = DebugTrace.trace ("TxInInfo Found: " ++ show info) ()
...
testRedeemer  _ getPolicy ctx (LedgerApiV2.Minting cs, redeemer) =
    let
        ...
        -- !txInfoMint = LedgerApiV2.txInfoMint $ LedgerApiV2.scriptContextTxInfo ctx
...
contractSBS :: LedgerApiV2.Validator -> SBS.ShortByteString
contractSBS validator = SBS.toShort . LBS.toStrict $ CodecSerialise.serialise contractScript
    where
        contractScript :: LedgerApiV2.Script
        contractScript = LedgerApiV2.unValidatorScript validator
...
createInValidRange date' = LedgerInterval.interval (date' - 1) (date' + 1 + (T.validTxTimeRange P.* 2))
