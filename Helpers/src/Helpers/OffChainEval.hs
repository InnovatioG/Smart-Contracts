...
createValidRange :: LedgerApiV2.POSIXTime -> LedgerApiV2.POSIXTime ->LedgerApiV2.POSIXTimeRange
createValidRange date' (LedgerApiV2.POSIXTime validTxTimeRangeInt) =
    let
        halfValid = validTxTimeRangeInt `divide` 2
    in LedgerInterval.interval (date' - LedgerApiV2.POSIXTime halfValid + 1) (date' + LedgerApiV2.POSIXTime halfValid -1)

createInValidRange :: LedgerApiV2.POSIXTime -> LedgerApiV2.POSIXTime -> LedgerApiV2.POSIXTimeRange
createInValidRange date' validTxTimeRange = LedgerInterval.interval (date' - 1) (date' + 1 + (validTxTimeRange P.* 2))
...
logAassertContainsAnyOf :: (P.Eq a, P.Show a, GHC.HasCallStack) => ([a], P.Either LedgerApiV2.EvaluationError LedgerApiV2.ExBudget, P.Either Integer Integer)  -> [a] -> Tasty.Assertion
logAassertContainsAnyOf (xs,_,_)  = assertContainsAnyOf xs
